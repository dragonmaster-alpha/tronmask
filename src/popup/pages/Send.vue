                ...
                <label class="input-label">
                    ...
                    <select class="input-field" v-model="selectedToken">
                        <option v-for="token in account.tokens" :key="token.name" :value="token">
                            {{ getTokenName(token) }} ({{ token.amount }} available)
                        </option>
                    </select>
                </label>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            console.log(this.account.tokens)
        },
        ...
        methods: {
            ...
            async sendPayment() {
                ...
                console.log(this.wallet.keypass, this.wallet.keystore);
                console.log('12312 ' + this.account.tokens.length);
                if (!wallet) {
                ...
                console.log(this.selectedToken, this.wallet.address, this.receipient, amount);

                try {
                    const result = await API().send(this.selectedToken.tokenName, this.wallet.address, this.receipient, amount)(wallet.privateKey)
